...
import (
	dpfm_api_input_reader "data-platform-api-production-order-confirmation-doc-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-production-order-confirmation-doc-reads-rmq-kube/DPFM_API_Output_Formatter"
	"fmt"
...
func (c *DPFMAPICaller) HeaderDoc(
	...
	if input.HeaderDoc.ProductionOrderConfirmation != nil {
		where = fmt.Sprintf("%s\nAND ProductionOrderConfirmation = %d", where, *input.HeaderDoc.ProductionOrderConfirmation)
	}
	...
	groupBy := "\nGROUP BY ProductionOrderConfirmation, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    ProductionOrderConfirmation, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
